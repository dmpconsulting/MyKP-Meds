...
public class PickListActivityTest {
    ...
    private ActivityController<PickListActivity> controller;
    private PickListActivity pickListActivity;

    @Before
    public void setup() {
        TestUtil.setupTestEnvironment();
        AppConstants.IS_NATIVE_RX_REFILL_REQUIRED = false;
        controller = Robolectric.buildActivity(PickListActivity.class);
        pickListActivity =  controller.create().start().resume().visible().get();
    }
    @After
    ...
    public void checkActivityNotNull(){
        assertNotNull(pickListActivity);
    }
}
