...
/**
 * Created by M1024581 on 7/19/2018.
 */


@Implements(RefillReminderDbHandler.class)
public class RefillReminderDBHandlerShadow {
    ...
    private final String testDataBase = "/database/mykpmeds-2018_refill.db";
    private String databasePath;

    public void __constructor__(Context context){
        try {
            databasePath = RefillReminderDbHandler.class.getResource(testDataBase).toURI().getPath();
    ...
    public Cursor executeRawQuery(String query, String[] selectionArgs) {
        ...
        // testDatabase.close();
        return testDatabase.rawQuery(query, selectionArgs);
    ...
    public Cursor getQuery(String table, String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy) {
        testDatabase =  SQLiteDatabase.openDatabase(new File(databasePath).getAbsolutePath(), null, version);
        Cursor cur= testDatabase.query(DatabaseConstants.PILL_TABLE, null, DatabaseConstants.PILL_USER_ID + "=?", selectionArgs, null, null, null);
        // testDatabase.close();
        return cur;
    }
    ...
    public Cursor executeQuery(String query) {
        testDatabase =  SQLiteDatabase.openDatabase(new File(databasePath).getAbsolutePath(), null, version);
        Cursor cur= testDatabase.rawQuery(query, null);
        //testDatabase.close();
        return cur;
